...
> **마감**: 2026년 7월 6일 (D-6) | 2차 심사: 7월 27~31일  
...
### D-6 / 6월 30일 (오늘) — 프론트엔드 통합 테스트

- [ ] **웹 E2E 테스트 3시나리오**
  - C25 금속가공 소기업 → 예측유지보수
  - C302 열처리 소기업 → 에너지절감
  - C243 주조 소기업 → 비전검사 (뿌리업종)
- [ ] **버그 수정**
  - ResultDashboard `assessment.includes` undefined 오류 → ✅ 수정 완료
  - ROIBarChart 빈 데이터 방어 처리 확인
- [ ] **UI 개선** (시간 있을 경우)
  - 진단 진행 단계 표시 개선
  - 에러 상태 UX 처리
...
- [ ] FastAPI 에러 핸들링 강화 (Gemini API 오류 시 graceful fallback)
- [ ] SSE 스트리밍 타임아웃 처리
- [ ] PDF 다운로드 한글 폰트 확인
- [ ] 환경변수 `.env.example` 정리
...
- [ ] 지원사업 데이터 실데이터 보강 (현재 5건 Mock → 최소 15건)
- [ ] 업종별 ROI 파라미터 세밀화
- [ ] Next.js 빌드(`npm run build`) 성공 확인

### D-2~1 / 7월 4~5일 — 제출 준비

- [ ] 서비스 데모 영상 또는 스크린샷 캡처
- [ ] 기획서.pdf 최종본 완성
- [ ] `README.md` 실행 가이드 작성
- [ ] 통합 테스트 최종 확인
