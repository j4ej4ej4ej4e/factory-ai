...
| 📅 지원금 마감 캘린더 | 자소설닷컴 채용캘린더 스타일 — 매칭된 지원사업(top_n 100건)을 월 그리드에 배치, 업종전용(파란점)/제조업 전체(회색점) 구분 표시, 날짜 클릭 시 상세 목록 + 자격 체크 버튼 | `subsidy_tool.py`(`is_industry_specific`), `SubsidyCalendar.tsx` |
| ✅ 지원사업 자격 자동 체크리스트 | 공고 description 원문을 회사 프로필과 대조해 충족/확인필요/미충족 판정 (온디맨드, 클릭 시에만 LLM 1회 호출) | `routers/subsidies.py`(`/subsidy-eligibility`) |
| ⚖️ RAG 성공/실패사례 균형 | 멀티쿼리 3→4개(실패사례·주의점 각도 추가), 리랭커가 case_type 분류, 실패사례 발견 시 최소 1건 결과에 강제 포함 | `retriever.py`, `ResultDashboard.tsx` |
| 🔍 판단 근거 트레이스 | "설명 가능한 AI" — pain_point→AI유형 규칙 매핑, 업종 기본 추천, RAG 검색 대상까지 전체 추론 과정을 화면에 그대로 노출 (블랙박스 아님) | `diagnostic.py`(`decision_trace`), `DecisionTrace.tsx` |

순위표는 조작된 통계가 아니라 실제 사용자 입력값이 쌓이는 구조라 서비스가 커질수록 정확해지는 성장형 지표. 백엔드(syntax+기능 테스트) / 프론트(tsc + `next build`) 전부 검증 완료.

**🐛 실사용 중 발견한 치명적 버그 2건 (수정 완료)**
- `InputForm.tsx`의 업종 드롭다운이 DB 시드 12개 업종과 안 맞았음(C20/C244/C303/C27/C28 등 존재하지 않는 코드 포함, C251/C259/C289/C301/C30 누락) — 이 상태로 선택하면 벤치마크가 통째로 빈 값이었음. 실제 12개로 재작성.
- pain_point 체크박스 8개 중 6개가 백엔드 `PAIN_POINT_TO_AI` 키와 이름이 달라(`energy_cost_high` vs `energy_cost` 등) 매핑이 조용히 실패하고 있었음 — 사용자가 체크해도 AI 우선순위에 반영이 안 됐음. 7개 키 전부 일치하도록 재작성.
- 겸사겸사 LLM 실패 시 폴백도 하드코딩 1건 → priority_ai 기반 개인화 Top3로 고도화.
...
| 2026-07-06 | v3.5 | 지원금 마감 캘린더 + 자격 체크리스트 + RAG 성공/실패 균형 + 판단 근거 트레이스(설명가능한 AI) 추가, 업종코드·pain_point 매핑 치명적 버그 2건 수정 |
| 2026-07-06 | v3.4 | 지원금 마감 캘린더 추가, RAG 리랭커 실패 시 관련도 0.0 표시 버그 수정(null로 변경) |
