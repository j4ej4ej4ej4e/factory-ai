...
| 📅 지원금 마감 캘린더 | 자소설닷컴 채용캘린더 스타일 — 매칭된 지원사업(top_n 20건)을 월 그리드에 배치, 업종전용(파란점)/제조업 전체(회색점) 구분 표시, 날짜 클릭 시 상세 목록. "업종 태깅된 공고가 21%뿐"이라는 실제 데이터 특성을 숨기지 않고 배지로 투명하게 드러냄 | `subsidy_tool.py`(`is_industry_specific`), `SubsidyCalendar.tsx` |
